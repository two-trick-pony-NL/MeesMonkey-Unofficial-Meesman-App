...
const CustomTabBar = (data, onLogout) => {
  ...
  const renderScreen = () => {
    switch (activeScreen) {
      case "Portfolio":
        return (
          <HomeScreen data={data.data} graphdata={data.data["historic_data"]} />
        );
      case "Rendementen":
        return <RendementScreen data={data["data"]} />;
      case "Transacties":
        return (
          <TransactionTable
            data={data.data["historic_data"]}
            to_invest={data.data}
          />
        );
      case "Profile":
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {activeScreen !== "LoginScreen" && (
        <FloatingNavBar setActiveScreen={setActiveScreen} />
      )}
    </View>
  );
};
